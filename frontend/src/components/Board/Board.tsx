...
function Board({
  ...
  const [answers, setAnswers] = useState<WeaponItem[][]>( Array.from(Array(num_rows), () => Array(num_cols).fill(null)) );
      ...
      <Grid2 container spacing={1}>
        ...
        {colLabels.map((label, colIndex) => (
          <Grid2 size={12 / (num_cols + 1)} key={`col-label-${colIndex}`}>
            <div style={{ textAlign: 'center', fontFamily: "monospace" }}>{label}</div>
          </Grid2>
        ))}
        ...
        {Array.from(Array(num_rows)).map((_, rowIndex) => (
          <React.Fragment key={`row-${rowIndex}`}>
            <Grid2 size={12 / (num_cols + 1)}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'right', height: '100%', fontFamily: "monospace" }}>
                {rowLabels[rowIndex]}
              </div>
            </Grid2>
            {Array.from(Array(num_cols)).map((_, colIndex) => (
              <Grid2 size={12 / (num_cols + 1)} key={`cell-${rowIndex}-${colIndex}`}>
                <Button
                  variant="outlined"
